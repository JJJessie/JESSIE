...
##用printf格式化日期：  
日期和时间格式用printf方法可以非常轻松地做到。您可以使用两个字母的格式，从t开始并在下面给出的表格中的其中一个字母结束。例如：
```
import java.util.Date;
...
|l|	两位数小时 (without leading zeroes), 1 和12之间|	6|
|M|	两位数分钟 (用前行零列)	|05|
|S|	两位数秒钟(用前行零列)	|19|
|L|	三位数毫秒用前行零列)	|047|
...
|3|	GregorianCalendar(int year, int month, int date, int hour, int minute) <br>构造一个GregorianCalendar用给定的日期和时间设置为与默认语言环境的默认时区。
